...
class PlateDetector(
    ...
    init {
        val modelBuffer = loadModelFile(context, modelSource)
        ...
        if (debugLogs) {
            Log.d("PlateDetector", "Loaded model=$modelSource")
    ...
    private fun loadModelFile(context: Context, modelSource: ModelSource): ByteBuffer {
    ...
    private fun loadAssetModel(context: Context, assetName: String): ByteBuffer {
    ...
    private fun loadUriModel(context: Context, uri: Uri): ByteBuffer {
        // Preferred: memory-map for file-backed URIs
        return try {
    ...
    companion object {
        /**
         * Cheap sanity check used by model discovery to reject a candidate file that isn't a
         * compatible detection model (e.g. an unrelated .tflite bundled by a dependency such as
         * ML Kit) before it's offered as a selectable model. Mirrors the shape checks in [init].
         */
        fun isValidModel(context: Context, modelSource: ModelSource): Boolean {
            return try {
                val buffer: ByteBuffer = when (modelSource) {
                    ...
                    is ModelSource.FilePath -> {
                        if (!modelSource.file.exists()) return false
                        FileInputStream(modelSource.file).use { fis ->
                            fis.channel.map(FileChannel.MapMode.READ_ONLY, 0, modelSource.file.length())
                        }
                    }
                    is ModelSource.ContentUri -> {
                        val bytes = context.contentResolver.openInputStream(modelSource.uri)
                ...
                try {
                    ...
                    if (inShape.size != 4 || inShape[0] != 1 || inShape[3] != 3) return false
                    if (interpreter.outputTensorCount < 1) return false
                    val outShape0 = interpreter.getOutputTensor(0).shape()
                    outShape0.size == 3 && outShape0[0] == 1 && outShape0[2] == 6
                } finally {
                    interpreter.close()
                }
            } catch (e: Exception) {
