...
the [`../android-training-data-collection-app`](../android-training-data-collection-app) app and promote the good ones into a
training-ready `done/` dataset. Never published to an app store; installed by sideloading the
debug/release APK.

It shares `android-training-data-collection-app`'s Cognito backend but accesses S3 **directly** through a scoped
...
3. **Configure** — copy `local.properties.example` to `local.properties` and fill in `COGNITO_*`
   from the stack outputs (same values as `../android-training-data-collection-app/local.properties`) and
