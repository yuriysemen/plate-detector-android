...
public [`../android-end-user-app/`](../android-end-user-app/CLAUDE.md) app (see [REQ-031](../requirements/REQ-031-done-split-detection-and-training-apps.md)).
It started as a full copy of `android-end-user-app/` (same detection/OCR/capture/upload pipeline, `applicationId`
`com.github.yuriysemen.platesdetector.training`) and is where all data-collection code now lives —
`android-end-user-app/` no longer has any of it. Live detection (YOLO TFLite + ML Kit OCR) still exists here
...
**On-device dataset editing was removed (REQ-026).** `DatasetEditorScreen`, `FrameDetailScreen`, `DatasetEditor`, `FrameEntry`, `YoloBox` are gone. The generic app only captures (auto / manual / burst, all shipping the model's predicted boxes) and uploads; review/editing lives in `android-training-data-reviewing-app`. Capture is gated on **`collect_training_data` AND signed in** — `LiveDetectionUi` computes `captureActive` and nothing is written to `training_data/` unless both hold.
