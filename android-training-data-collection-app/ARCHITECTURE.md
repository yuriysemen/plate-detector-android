...
MainActivity
  └── LivePlateDetectionScreen          (top-level coordinator)
        ├── BackendConfigScreen         (REQ-032 — view/edit COGNITO_*/UPLOAD_SERVICE_URL on-device;
        │                                reachable from Settings, or via NoModelsScreen's always-visible
        │                                "Configure backend…" link, or auto-routed to on sign-in attempt
        │                                when unconfigured)
        ├── NoModelsScreen              (no .tflite bundled/downloaded; carries Sign in / Sign out and
        │                                "Configure backend…" so a fresh install / different-user /
        │                                reconfigure isn't a dead end)
        ...
        │                                otherwise a "Sign in" row; "Backend configuration" row always
        │                                shown; a "session expired" card when applicable)
...
`LivePlateDetectionScreen` owns the routing state (`showSettings`, `showExport`, `showAuth`, `showBackendConfig`, `isModelEnabled`, `selectedId`). `showBackendConfig` takes priority over `showAuth` in the `when` block — both render from anywhere in the flow, **including over `NoModelsScreen`**. A `requestSignIn()` helper checks `AppConfig.isBackendConfigured()` and routes to `BackendConfigScreen` instead of `AuthScreen` when unconfigured, so sign-in is never attempted against an empty/invalid pool. When no model is selected on first launch it opens Settings automatically. `ContributeScreen` is shown when `showExport` is true.

On-device dataset review/editing was **removed** (REQ-026) — the generic app only captures and uploads; all box editing now lives in the separate `android-training-data-reviewing-app` app. `DatasetEditorScreen`, `FrameDetailScreen`, and `DatasetEditor` are gone.
...
**Capture guidelines gate (REQ-038).** `LiveDetectionUi` checks `ModelPrefs.getCaptureGuidelinesShown()` as the very first thing in the composable — before the CAMERA permission `LaunchedEffect` and before `TrainingDataSaver`/`DatasetExporter` are created. If not yet acknowledged, it renders `CaptureGuidelinesScreen` (field-technique tips: use Manual Capture for what auto-capture misses, vary angle/distance, seek out hard cases, favor more vehicles over duplicate shots, stay public/respectful) and returns, so nothing camera-related runs until the collector taps "Got it — start capturing". Persisted via `ModelPrefs.setCaptureGuidelinesShown` (new `capture_guidelines_shown` key), shown once per install regardless of `collect_training_data`/sign-in state.
...
- **Analysis resolution** — `AnalysisResolution` enum (`LOW` 640×480 / `HD` 1280×720 only, REQ-042) persisted in `ModelPrefs` (legacy `DEFAULT` → `LOW`); wired into `ImageAnalysis.Builder` and `Preview` via `ResolutionSelector` with a pinned `AspectRatioStrategy` (4:3 / 16:9) + `ResolutionStrategy` on the analysis use case; camera is fully rebound when changed (via `key(spec.id, analysisResolution)`); picker shown in `SettingsScreen`
...
| `ModelPrefs` | `LivePlateDetectionScreen.kt` | SharedPreferences wrapper; keys: selected model, per-model conf, show-labels, `collect_training_data`, `collect_first_time_shown`, `capture_guidelines_shown`, analysis resolution, `scan_interval_ms` (default 1000), storage quota |
...
the ZIP is deleted on success. `android-training-data-reviewing-app` reviews the uploaded packages.
...
standalone `android-training-data-reviewing-app` app, which consumes the uploaded YOLO packages from S3.
