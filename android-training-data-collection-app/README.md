...
for the detection model. It's a copy of the published [`../android-end-user-app/`](../android-end-user-app/README.md) app
that keeps the full sign-in / capture / cloud-upload pipeline `android-end-user-app/` no longer has — see
[REQ-031](../requirements/REQ-031-done-split-detection-and-training-apps.md) for why the two were
split. Detected/captured frames upload to the same S3 bucket `android-training-data-reviewing-app` reads from.

## Quick start

1. **Configure** — either:
   - copy `local.properties.example` to `local.properties` and fill in the `COGNITO_*` /
     `UPLOAD_SERVICE_URL` values from the [`aws-training-infra/aws`](../aws-training-infra/aws/README.md) stack outputs
     (same values as `android-training-data-reviewing-app/local.properties`'s `COGNITO_*`), then rebuild; or
...
- [`ROADMAP.md`](ROADMAP.md) — full feature history (inherited from `android-end-user-app/` as of the split).
- [`../android-training-data-reviewing-app/`](../android-training-data-reviewing-app/README.md) — where the uploads from this app get
