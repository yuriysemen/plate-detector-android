...
- [ ] **Upload authentication — Android client** — sign-up / sign-in / verify screens; SRP auth via Cognito SDK; ID token → Identity Pool STS credentials; SigV4-sign `POST /get-upload-url`; persist refresh token; 4 new Settings fields (Upload URL, User Pool ID, App Client ID, Identity Pool ID) (REQ-014)
