...
6. `ContributeScreen` — shows a stats card (frames collected, total detections, storage used with ✏ quota edit; "View dataset" and "Reset collected data" buttons), storage quota banners (80% warning / 100% pause), upload configuration card (auth status row with sign-in/out, mobile data toggle, daily auto-upload time, last auto-upload date), "Upload collected data" button (disabled when not signed in or no frames), and an "Upload history" section listing all upload entries — active (ZIP present), failed (retry button), and completed (UPLOADED, sidecar-only, shows frame count + upload date). UPLOADED items persist indefinitely; ZIP is deleted on success, sidecar kept as the history record. Upload URL and Cognito config IDs are embedded at build time via `BuildConfig`/`AppConfig` — no UI fields for them.
