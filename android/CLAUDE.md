...
6. `ContributeScreen` — shows a stats card (frames collected, total detections; with "View dataset" button on the right and "Reset collected data" button centered at the bottom), storage limit setting, upload config (server URL, mobile data toggle), storage quota banners (80% warning / 100% pause), "Upload collected data" button, and a conditional "Session in progress" section listing active/failed upload jobs.
