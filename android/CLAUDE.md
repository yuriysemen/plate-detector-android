...
6. `ContributeScreen` — shows collection stats, storage limit setting, upload config (server URL, mobile data toggle), storage quota banners (80% warning / 100% pause), action buttons (Edit / Upload / Reset), and a conditional "Session in progress" section listing active/failed upload jobs.
