...
> **Partially split by [REQ-031](REQ-031-done-split-detection-and-training-apps.md):** the detection pixel pipeline below (camera → rotate → `PlateDetector`) is unchanged and still exactly how `android-end-user-app/` works today. Only the YOLO-normalized annotation conversion ("Conversion to YOLO format" below, `TrainingDataSaver.saveFrame()`) is `android-training-data-collection-app/`-only now — `android-end-user-app/` has no annotation-saving code at all.
